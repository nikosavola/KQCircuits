...
def copy_palace_scripts_to_directory(path: Path):
    """
    Copies Palace scripts into directory path.
    TODO factor all copy scripts to accept `script_path`.
...
def export_palace_script(json_filenames, path: Path, workflow=None, file_prefix='simulation',
    ...
    with open(main_script_filename, 'w') as main_file:
        ...
        for i, json_filename in enumerate(json_filenames):
            ...
            with open(script_filename, 'w') as file:
                ...
                file.write('echo "Simulation {}/{} Palace"\n'.format(i + 1, len(json_filenames)))
                file.write('{2} "{0}" "{1}" --only-palace 2>&1 >> "{1}_Palace.log"\n'.format(
                    script_file,
                    Path(json_filename).relative_to(path),
                    python_executable)
                )
...
def export_palace(simulations: List,
    ...
    copy_palace_scripts_to_directory(path)
    json_filenames = []
    for simulation in simulations:
        try:
            json_filenames.append(export_palace_json(simulation, path, tool, linear_system_method,
    ...
    return export_palace_script(json_filenames, path, workflow, file_prefix=file_prefix, script_file=script_file)
